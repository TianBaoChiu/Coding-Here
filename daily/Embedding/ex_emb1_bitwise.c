...
#define SET_BIT(REG, BIT)     (REG |= 1 << BIT)/* 將 REG 的第 BIT 位元設為 1 */
#define CLEAR_BIT(REG, BIT)   (REG &= ~(1 << BIT))/* 將 REG 的第 BIT 位元設為 0 */
#define TOGGLE_BIT(REG, BIT)  (REG ^= 1 << BIT)/* 將 REG 的第 BIT 位元反轉 */
#define CHECK_BIT(REG, BIT)   ((REG >> BIT) & 1)/* 檢查 REG 的第 BIT 位元是否為 1 (回傳 0 或非 0) */
...
int main() {
    ...
    if(CHECK_BIT(PORTA, 7)) {printf("Bit 7 is SET\n");}
    if(CHECK_BIT(PORTA, 3)) {printf("Bit 3 is SET\n");}
